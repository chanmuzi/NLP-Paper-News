...
- 워크플로우:
  ...
  - `.github/workflows/post-approved.yml` — 승인된 스레드를 X에 게시하고, 성공 시 `deploy.yml`을 트리거해 사이트에 반영합니다 (`ENABLE_X_POST` 변수로 on/off).
- 게시 실패 시 수동 포스팅용 초안 아티팩트(`social-draft.md`)를 남깁니다.
- `deploy.yml`은 `data/items.json`만 바뀐 push는 자동 배포하지 않으므로(admin 승인 전 조기 배포 방지), 두 게시 흐름 모두 게시 성공 후 배포를 명시적으로 트리거합니다.
...
- **흐름**: URL 요약(마크다운) → X 메인+리플라이+가중 글자수 → 사용자 응답(`게시` / `드라이런` / `수정: ...`) → 게시 + 푸시 + `deploy.yml` 트리거(사이트 자동 반영).
