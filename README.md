...
- `data/items.json` 변경 시 신규 항목을 감지하여 X 스레드를 자동 게시합니다.
- 워크플로우: `.github/workflows/notify.yml` (`ENABLE_X_POST` 변수로 on/off)
...
- **안전장치**: 280자 가중 검증(`scripts/x-length.mjs`), 게시 실패 시 자동 롤백 또는 `main_only` degraded mode.
