...
```
[admin UI 흐름]
admin 페이지에서 항목 추가 → Git Data API로 main에 commit/push
    → admin UI가 notify.yml workflow_dispatch 명시 호출
    → notify.yml(Prepare X Thread Draft): extract-new-items → build-digest → 아티팩트 업로드
    → 웹 UI에서 승인
...
## Workflow / CI 변경 시 동기화

`.github/workflows/*.yml`의 정의(트리거/이름/inputs)를 변경할 때는 다음 파일·위치도 같은 PR에서 함께 갱신한다. 한 군데라도 누락되면 docs와 실제 동작이 어긋난다.

**갱신 대상 (사실 정합성)**
- `CLAUDE.md` — 위 "X 포스팅 파이프라인" 흐름도, "CI/CD" 섹션
- `README.md` — 자동 X 포스팅 섹션 (트리거/스위치 설명)
- `docs/notification-runbook.md` — 트리거 목록, 테스트 절차
- `.claude/commands/post.md` — Step 4c push trigger 동작 설명
- `web/src/pages/admin.astro` — 워크플로우 이름 비교(`run.name === '...'`), 폴링 fallback 쿼리(`event=...`), `dispatches` 호출 경로

**변경 직전 직접 확인 (Verify Before Acting)**
1. 대상 워크플로우의 모든 trigger (`on.push.paths`, `on.pull_request.paths`, `on.workflow_dispatch.inputs`) 정의를 파일에서 직접 읽는다 — 추정·메모리에 의존하지 않는다.
2. 의존하는 다른 워크플로우(예: `deploy.yml`의 `paths-ignore`, `post-approved.yml`의 inputs) 정의를 확인한다.
3. admin UI(`web/src/pages/admin.astro`)에서 그 워크플로우 이름·이벤트로 폴링·매칭하는 위치를 grep으로 찾아 함께 갱신한다.
